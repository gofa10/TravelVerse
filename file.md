...
═══════════════════════════════════════════════════════════════════════════════
STATE DIAGRAM ANALYSIS (Comprehensive)
═══════════════════════════════════════════════════════════════════════════════
...
|--------|-------|------------|------------|-------------|
| POST | /register | AuthController@register | guest.only | User registration |
| POST | /login | AuthController@login | guest.only | User login |
| POST | /forgot-password | ForgotPasswordController@sendResetLinkEmail | guest.only | Password reset request |
| POST | /reset-password | ResetPasswordController@reset | guest.only | Reset password |
| POST | /logout | AuthController@logout | auth:sanctum | Logout |
| GET | /profile | AuthController@profile | auth:sanctum | Get user profile |
| GET | /user | UserController@profile | auth:sanctum | Get user data |
| POST | /update-profile | UserController@updateProfile | auth:sanctum | Update profile |
| DELETE | /profile/avatar | UserController@removeAvatar | auth:sanctum | Remove avatar |
| POST | /change-password | UserController@changePassword | auth:sanctum | Change password |
| DELETE | /delete-account | UserController@deleteAccount | auth:sanctum | Delete account |

### Middleware Configuration (app/Http/Kernel.php)

| Middleware Name | Class | Purpose |
|-----------------|-------|---------|
| auth:sanctum | Laravel\Sanctum\Http\Middleware\EnsureFrontendRequestsAreStateful | Token-based authentication |
| guest.only | App\Http\Middleware\GuestOnly | Prevents authenticated users from accessing login/register |
| usertype | App\Http\Middleware\CheckUserType | Role-based access control |

### GuestOnly Middleware (app/Http/Middleware/GuestOnly.php:11-21)
- If auth()->check() returns true, returns 403 response with message: "You are already authenticated."
- Otherwise, allows request to proceed

### CheckUserType Middleware (app/Http/Middleware/CheckUserType.php:11-23)
- Valid user_type values: 'user', 'admin', 'tour_guide'
- Returns 403 with message: "Unauthorized" if user type doesn't match required types

### Frontend Role-Based Redirect (RedirectByRole.jsx:20-27)
- Token OK + !user + (pathname === '/' || pathname === '/login'):
  - dispatch(getProfile()).then((res) => {
    - role === 'admin' → navigate('/admin')
    - role === 'tour_guide' → navigate('/guide')
    - role === 'user' → navigate('/')
  - })

### Token Validation (Utility/authToken.js:1-18)
- getToken(): Retrieves 'token' from localStorage
- isValidToken(): Checks token exists and is not 'undefined' or 'null'
- hasValidToken(): Returns boolean for token validity
- clearAuth(): Removes 'token' from localStorage

### Login Flow (authSlice.js:6-14)
- Endpoint: POST /login
- Success: Stores token in localStorage, returns user data
- Error: Returns 'Login failed' or server message

### After Login Redirects
| Source | Role | Redirect Path |
|--------|------|---------------|
| RedirectByRole.jsx | admin | /admin |
| RedirectByRole.jsx | tour_guide | /guide |
| RedirectByRole.jsx | user | / |
| Login.jsx (register) | admin | /admin |
| Login.jsx (register) | other | / |

---

## 2. ROLE-BASED STATES (user_type values)

### Database Schema (database/migrations/0001_01_01_000000_create_users_table.php:20)
- $table->enum('user_type', ['user', 'tour_guide', 'admin'])->default('user');

### Valid user_type Values
| Value | Description | Default |
|-------|-------------|---------|
| user | Regular traveler | Yes |
| tour_guide | Trip creator/manager | No |
| admin | System administrator | No |

### Frontend Role Protection (RoleProtectedRoute.jsx:6-14)
- Props: allowedRoles (array of user_type strings)
- If no user: Navigate to /login
- If user.user_type not in allowedRoles: Navigate to /

### App.jsx Route Configuration (frontend/src/App.jsx:107-165)

**Admin Routes (allowedRoles=['admin']):**
- /admin → AdminDash with nested routes:
  - /admin/ → Main (dashboard stats)
  - /admin/users → UserManagement
  - /admin/flights → FlightManagement
  - /admin/trips → TripManagement
  - /admin/hotel → HotelManagement
  - /admin/restaurants → RestaurantManagement
  - /admin/activities → ActivityManagement
  - /admin/cars → CarManagement
  - /admin/cruises → CruiseManagement
  - /admin/reservations → AdminReservations

**User Routes (allowedRoles=['user']):**
- /user → Dashboard with nested routes:
  - /user/ → Overview
  - /user/reservations → MyReserv
  - /user/favorites → MyFavorites
  - /user/profile → UserProfileSettings
  - /user/reviews → UserReviews

**Guide Routes (allowedRoles=['tour_guide']):**
- /guide → GuideDashboard with nested routes:
  - /guide/ → Navigate to trips
  - /guide/trips → GuideMyTrips
  - /guide/reservations → GuideMyReservations
  - /guide/reviews → GuideMyReviews
  - /guide/profile → GuideProfile

**Public Routes (no protection):**
- /, /destination/:continent, /city/:cityName, /trips, /TravelBook, /itemdetail/:id, /hotel/:cityName, /car/:cityName, /thingstodo/:cityName, /allactivites/:cityName, /flight/:cityName, /cruises/:cityName, /restaurants/:cityName, /forgotPassword, /reset-password, /search

---

## 3. BOOKING / RESERVATION STATES

### Database Status Values (from migrations)

**Current ENUM (2026_03_09_140000_add_left_without_booking_status_to_reservations.php:14):**
- ENUM('saved','redirect_pending','booking_claimed','booking_declined','left_without_booking','cancelled') DEFAULT 'saved'

### Reservation Status Values
| Status | Description | Default |
|--------|-------------|---------|
| saved | Initial state when item added to cart/saved | Yes |
| redirect_pending | User clicked "Book Now" and was redirected | No |
| booking_claimed | User confirmed booking completion | No |
| booking_declined | User declined/not completed booking | No |
| left_without_booking | User closed modal without responding | No |
| cancelled | Reservation cancelled by user | No |

### Status Transitions (ReservationController.php:125-132)
...
|------|-----|---------|------------|
| saved | redirect_pending | User clicks "Book Now" | BookingModal.jsx:119 |
| saved | cancelled | User clicks "Cancel" | MyReservations.jsx:84 |
| redirect_pending | booking_claimed | User clicks "Yes, I booked it!" | BookingModal.jsx:151 |
| redirect_pending | booking_declined | User clicks "Not yet" | BookingModal.jsx:152 |
| redirect_pending | left_without_booking | 30s timeout or tab closed | BookingModal.jsx:87 |
| any | cancelled | DELETE /reservations/{id} | ReservationController@destroy |

### API Endpoints for Reservations

| Method | Route | Controller | Middleware | Description |
|--------|-------|------------|------------|-------------|
| GET | /reservations | ReservationController@index | usertype:user | List user reservations |
| POST | /reservations | ReservationController@store | usertype:user | Create reservation |
| PATCH | /reservations/{id}/status | ReservationController@updateStatus | usertype:user | Update status |
| DELETE | /reservations/{id} | ReservationController@destroy | usertype:user | Cancel reservation |
| GET | /guide/reservations | ReservationController@guideReservations | usertype:tour_guide | Guide's trip reservations |
| GET | /admin/reservations | ReservationController@adminIndex | usertype:admin | All reservations |

### Email Notifications
| Trigger | Notification Class | Recipient |
|---------|-------------------|-----------|
| Status → booking_claimed | ReservationConfirmed | $reservation->user |
| DELETE /reservations/{id} | ReservationCanceled | $reservation->user |

### Reservable Types (polymorphic)
- App\Models\Trip
- App\Models\Hotel
...
## 4. USER ACTIONS PER ROLE

### ADMIN Actions

**Pages/Components Access:**
- Main.jsx (dashboard stats)
- UserManagement.jsx
...
- CarManagement.jsx
- CruiseManagement.jsx
- AdminReservations.jsx

**API Endpoints (from routes/api.php:139-194):**

| Method | Endpoint | Success Message | Error Message |
|--------|----------|-----------------|---------------|
| GET | /admin/dashboard | Returns stats | 403 Unauthorized |
| GET | /admin/reservations | Returns paginated list | 403 Unauthorized |
| GET | /users | Returns users | 403 Unauthorized |
| GET | /users/{id}/details | Returns user details | 404 Not found |
| POST | /users | User created | 422 Validation error |
| PUT | /users/{id} | User updated | 404 Not found |
| DELETE | /users/{id} | User deleted | 404 Not found |
| GET | /users/guides | Returns guides | 403 Unauthorized |
| POST | /hotels | Hotel created | 422 Validation error |
| PUT | /hotels/{hotel} | Hotel updated | 404 Not found |
| DELETE | /hotels/{hotel} | Hotel deleted | 404 Not found |
| POST | /trips | Trip created | 422 Validation error |
| PUT | /trips/{trip} | Trip updated | 404 Not found |
| DELETE | /trips/{trip} | Trip deleted | 404 Not found |
| POST | /activities | Activity created | 422 Validation error |
| PUT | /activities/{activity} | Activity updated | 404 Not found |
| DELETE | /activities/{activity} | Activity deleted | 404 Not found |
| POST | /cruises | Cruise created | 422 Validation error |
| PUT | /cruises/{cruise} | Cruise updated | 404 Not found |
| DELETE | /cruises/{cruise} | Cruise deleted | 404 Not found |
| POST | /restaurants | Restaurant created | 422 Validation error |
| PUT | /restaurants/{restaurant} | Restaurant updated | 404 Not found |
| DELETE | /restaurants/{restaurant} | Restaurant deleted | 404 Not found |
| POST | /cars | Car created | 422 Validation error |
| PUT | /cars/{car} | Car updated | 404 Not found |
| DELETE | /cars/{car} | Car deleted | 404 Not found |
| POST | /flights | Flight created | 422 Validation error |
| PUT | /flights/{flight} | Flight updated | 404 Not found |
| DELETE | /flights/{flight} | Flight deleted | 404 Not found |
| DELETE | /reviews/{review} | Review deleted | 404 Not found |
| POST | /images | Image uploaded | 422 Validation error |
| DELETE | /images/{id} | Image deleted | 404 Not found |
...
- GuideDashboard.jsx
- GuideMyTrips.jsx
- GuideMyReservations.jsx
- GuideMyReviews.jsx
- GuideProfile.jsx

**API Endpoints (from routes/api.php:123-136):**

| Method | Endpoint | Success Message | Error Message |
|--------|----------|-----------------|---------------|
| GET | /guide/trips | Returns paginated trips | 403 Forbidden |
| POST | /guide/trips | Trip created successfully | Failed to create trip |
| PUT | /guide/trips/{trip} | Trip updated successfully | Failed to update trip |
| DELETE | /guide/trips/{trip} | Trip deleted successfully | Failed to delete trip |
| GET | /guide/reservations | Returns paginated reservations | Failed to load reservations |
| GET | /guide/reviews | Returns paginated reviews | Failed to load reviews |
| POST | /guide/reviews/{review}/reply | Reply saved successfully | Failed to save reply |

### USER (Regular) Actions
...
**API Endpoints (from routes/api.php:104-121):**

| Method | Endpoint | Success Message | Error Message |
|--------|----------|-----------------|---------------|
| GET | /favorites | Returns favorites list | Failed to load favorites |
| POST | /favorites | Favorite added | Already in favorites (409) |
| DELETE | /favorites/{id} | Removed from favorites | Failed to remove |
| GET | /my-reviews | Returns user's reviews | Failed to load |
| POST | /reviews | Review added | Failed to add review |
| GET | /reservations | Returns reservations | Failed to load |
| POST | /reservations | Reservation created | Failed to create |
| PATCH | /reservations/{id}/status | Status updated | Invalid status transition |
| DELETE | /reservations/{id} | Reservation cancelled | Failed to cancel |

### Toast/Success Messages Summary
...
- Account Delete: "Account deleted successfully"
...
- Reply Add: "Reply saved successfully"
- Favorite Add: "Added to favorites"
...
- Register: "Registration failed. Check your data."
...
### Model (app/Models/Favorite.php:8-27)
...
### Favoritable Types (FavoriteController.php:29-43)
- Trip, Hotel, Restaurant, Activity, Cruise, Car, Flight

### API Endpoints
| Method | Route | Controller | Response |
|--------|-------|------------|----------|
| GET | /favorites | FavoriteController@index | List of favorites with favoritable |
| POST | /favorites | FavoriteController@store | 201 Created with loaded favoritable |
| DELETE | /favorites/{id} | FavoriteController@destroy | 200 OK with message "Deleted" |

### Frontend States (MyFavorites.jsx)
- Loading: Shows spinner
- Error: "Failed to load favorites."
- Empty: "No favorites found."
- Success: Grid of favorite cards with type filter (all, Trip, Hotel, Restaurant, Activity)

### Success/Error Messages
| Action | Success | Error |
|--------|---------|-------|
| Add | "Added to favorites" | "Already in favorites" (409) |
| Remove | "Removed from favorites" | "Failed to remove" |

---

## 6. GUIDE-SPECIFIC STATES

### Trip Management (GuideMyTrips.jsx)

**States:**
- View List: Table of guide's trips with reviews count
- Create Mode: Modal form for new trip
- Edit Mode: Modal form for existing trip

**Form Fields:**
- name_en (required), name_ar (required), description_en, description_ar
- location (required), price (required, numeric), rate (required, 0-5)
- duration, continent, difficulty (easy/medium/hard), booking_link
- images[] (file upload)

**API Endpoints:**
| Action | Method | Endpoint | Success | Error |
|--------|--------|----------|---------|-------|
| List | GET | /guide/trips | Returns paginated | Failed to load |
| Create | POST | /guide/trips | 201 Created | Failed to create trip |
| Update | PUT | /guide/trips/{id} | Updated | Failed to update trip |
| Delete | DELETE | /guide/trips/{id} | Deleted | Failed to delete trip |

### Reservation Viewing (GuideMyReservations.jsx)

**States:**
- Loading: "Loading reservations..."
- Error: "Failed to load reservations."
- Empty: "No reservations yet"
- Success: Table with columns: Traveler, Trip, Date, People, Status

**API Endpoint:**
| Action | Method | Endpoint | Response |
|--------|--------|----------|----------|
| List | GET | /guide/reservations | Paginated reservations for guide's trips |

### Review Response (GuideMyReviews.jsx)

**States:**
- Loading: "Loading reviews..."
- Error: "Failed to load reviews."
- Empty: "No reviews yet"
- With Reply: Shows reply in blue box
- Without Reply: Textarea for drafting reply

**API Endpoints:**
| Action | Method | Endpoint | Success | Error |
|--------|--------|----------|---------|-------|
| List | GET | /guide/reviews | Returns reviews | Failed to load |
| Reply | POST | /guide/reviews/{id}/reply | "Reply saved" | Failed to save |

---

## 7. ADMIN-SPECIFIC STATES

### Dashboard Stats (Main.jsx from AdminDash)
...
7. Cars (CRUD)
8. Flights (CRUD)
9. Reviews (Delete only)
10. Reservations (View only)
11. Images (Upload/Delete)

### User Management (UserMang/)

**States:**
- List View: Paginated users with pagination
- Create Mode: Form for new user
- Edit Mode: Form for existing user
- Delete: Confirmation dialog

**Fields:**
- name (required), email (required, unique), user_type (user/admin/tour_guide)
- password (required for create), avatar (optional image)

**API Endpoints:**
| Action | Method | Endpoint | Success | Error |
|--------|--------|----------|---------|-------|
| List | GET | /users | Paginated list | 403 |
| All | GET | /users/all | All users | 403 |
| Guides | GET | /users/guides | Tour guides | 403 |
| Details | GET | /users/{id}/details | Full details | 404 |
| Create | POST | /users | User created | 422 |
| Update | PUT | /users/{id} | User updated | 404/422 |
| Delete | DELETE | /users/{id} | User deleted | 404 |

### Reservation Management (AdminReservations.jsx)

**States:**
- Loading: "Loading reservations..."
- Error: "Failed to load reservations."
- Success: Table with status counts

**Stats Displayed:**
- Saved, Link Opened, Booked ✓, Not Booked, No Response, Conversion %, Interest Rate %

**API Endpoint:**
| Action | Method | Endpoint | Response |
|--------|--------|----------|----------|
| List | GET | /admin/reservations | Paginated with counts |

### CRUD Operations for All Entities

**Trip Management:** Create POST /trips, Update PUT /trips/{trip}, Delete DELETE /trips/{trip}, Upload Images POST /trips/{id}/images
**Hotel Management:** Create POST /hotels, Update PUT /hotels/{hotel}, Delete DELETE /hotels/{hotel}
**Restaurant Management:** Create POST /restaurants, Update PUT /restaurants/{restaurant}, Delete DELETE /restaurants/{restaurant}
**Activity Management:** Create POST /activities, Update PUT /activities/{activity}, Delete DELETE /activities/{activity}, Get Types GET /activities/types
**Cruise Management:** Create POST /cruises, Update PUT /cruises/{cruise}, Delete DELETE /cruises/{cruise}
**Car Management:** Create POST /cars, Update PUT /cars/{car}, Delete DELETE /cars/{car}
**Flight Management:** Create POST /flights, Update PUT /flights/{flight}, Delete DELETE /flights/{flight}
**Review Management:** Delete DELETE /reviews/{review}
**Image Management:** Upload POST /images, Delete DELETE /images/{id}
...
- GUEST --login--> AUTHENTICATED (token stored)
- AUTHENTICATED --logout--> GUEST
- AUTHENTICATED --role check--> ADMIN (/admin)
- AUTHENTICATED --role check--> GUIDE (/guide)
- AUTHENTICATED --role check--> USER (/user)

### Reservation State Machine
- saved (default) --Book Now--> redirect_pending
- saved --Cancel--> cancelled
- redirect_pending --"Yes, I booked it!"--> booking_claimed
- redirect_pending --"Not yet"--> booking_declined
- redirect_pending --30s timeout/tab close--> left_without_booking
- redirect_pending --Cancel--> cancelled
- booking_claimed --Cancel--> cancelled
- booking_declined --re-try--> redirect_pending
- booking_declined --Cancel--> cancelled
- left_without_booking --re-try--> redirect_pending
- left_without_booking --Cancel--> cancelled

### Favorite State Machine
- EMPTY --Add--> FAVORITED --Remove--> EMPTY (toggle)

### User Account State Machine
- ACTIVE --update--> UPDATED --change password--> PASSWORD_CHANGED
- ACTIVE --delete--> DELETED (logged out)

═══════════════════════════════════════════════════════════════════════════════
END OF STATE DIAGRAM ANALYSIS
═══════════════════════════════════════════════════════════════════════════════
