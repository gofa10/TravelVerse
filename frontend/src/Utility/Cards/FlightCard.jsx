...
const StopsTag = ({ stops }) => {
  const colors = { 0: "#28a745", 1: "#fd7e14" };
  const labels = { 0: "Direct ✓", 1: "1 Stop" };
  const color = colors[stops] ?? "#dc3545";
...
const FlightDetailModal = ({ data, onClose }) => {
  if (!data) return null;
  const convertedPrice = data.price_per_person || data.price || 0;
      ...
      >
        ...
        <div style={{
          ...
          <div className="d-flex align-items-center gap-3">
            {data.airline_logo && (
              <img src={data.airline_logo} alt={data.airline}
                style={{ width: 44, height: 44, objectFit: "contain", borderRadius: "8px", background: "#fff", padding: "4px" }} />
            )}
            <div>
              <h5 style={{ color: "#fff", margin: 0, fontWeight: 700 }}>{data.airline}</h5>
        ...
        <div style={{ padding: "24px" }}>
          ...
          <div className="d-flex align-items-center justify-content-between mb-4">
            <div className="text-center">
              <div style={{ fontSize: "36px", fontWeight: 800, lineHeight: 1, color: "#1a1a2e" }}>
                {fmtTime(data.departure_time)}
              </div>
              <div style={{ fontSize: "12px", color: "#888", marginTop: "2px" }}>{fmtDate(data.departure_time)}</div>
              <div style={{ fontSize: "22px", fontWeight: 700, color: "#0f3460", marginTop: "6px" }}>{data.from_code}</div>
              <div style={{ fontSize: "12px", color: "#666", maxWidth: "120px", textAlign: "center" }}>{data.from_airport}</div>
            </div>
            ...
            <div style={{ flex: 1, textAlign: "center", padding: "0 16px" }}>
              ...
              <div style={{ position: "relative", borderTop: "2px dashed #adb5bd" }}>
                <span style={{
            ...
            <div className="text-center">
              ...
              <div style={{ fontSize: "12px", color: "#888", marginTop: "2px" }}>{fmtDate(data.arrival_time)}</div>
              <div style={{ fontSize: "22px", fontWeight: 700, color: "#0f3460", marginTop: "6px" }}>{data.to_code}</div>
              <div style={{ fontSize: "12px", color: "#666", maxWidth: "120px", textAlign: "center" }}>{data.to_airport}</div>
            </div>
          </div>
          ...
          {data.carbon_emissions && (
            <div style={{
              ...
              fontSize: "13px", color: "#166534", marginBottom: "20px",
            }}>
          ...
          <div className="d-flex align-items-center justify-content-between">
            <div>
              <div style={{ fontSize: "32px", fontWeight: 800, color: "#0f3460" }}>
                ${convertedPrice}
                <span style={{ fontSize: "14px", color: "#888", fontWeight: 400, marginLeft: "6px" }}>/ person</span>
              </div>
            </div>
            <div className="d-flex gap-2 flex-wrap">
              ...
              <button
                ...
                style={{
                  background: "linear-gradient(135deg,#0770e3,#05a9f7)",
                  color: "#fff", border: "none", borderRadius: "12px",
                  padding: "12px 18px", fontWeight: 700, fontSize: "13px",
                  cursor: "pointer",
                }}
              ...
              <MDBCardBody style={{ padding: "18px 22px" }}>
                <MDBRow className="align-items-center g-3">
                  ...
                  <MDBCol xs={12} sm={2} className="d-flex align-items-center gap-2">
                    {data.airline_logo
                      ? <img src={data.airline_logo} alt={data.airline}
                        style={{ width: 38, height: 38, objectFit: "contain", borderRadius: "8px" }}
                    ...
                    <div>
                      <div className="fw-bold dark:text-white!" style={{ fontSize: "13px" }}>
                        {data.airline || "Airline"}
                      </div>
                      <div style={{ fontSize: "11px", color: "#999" }}>{data.flight_number}</div>
                    </div>
                  </MDBCol>
                  ...
                  <MDBCol xs={12} sm={5} className="d-flex align-items-center">
                    <div className="text-center">
                      ...
                      <div style={{ fontSize: "11px", color: "#888" }}>{fmtDate(data.departure_time)}</div>
                      <div className="fw-semibold dark:text-white/70!" style={{ fontSize: "14px", color: "#333" }}>
                        {data.from_code}
                      </div>
                    </div>
                    ...
                    <div className="flex-grow-1 text-center mx-3">
                      ...
                      <div style={{ borderTop: "2px dashed #adb5bd", position: "relative" }}>
                        <span style={{
                    ...
                    <div className="text-center">
                      ...
                      <div style={{ fontSize: "11px", color: "#888" }}>{fmtDate(data.arrival_time)}</div>
                      <div className="fw-semibold dark:text-white/70!" style={{ fontSize: "14px", color: "#333" }}>
                        {data.to_code}
                      </div>
                    </div>
                  ...
                  <MDBCol xs={6} sm={2} className="text-center">
                    <span style={{
                      background: data.travel_class === "Business" ? "#fff3cd" : "#e8f4ee",
                      color: data.travel_class === "Business" ? "#856404" : "#155724",
                  ...
                  <MDBCol xs={6} sm={3} className="text-center border-start">
                    <div className="d-flex align-items-baseline justify-content-center gap-2 mb-1">
                      <span className="dark:text-yellow-400!" style={{ fontSize: "22px", fontWeight: 800, color: "#0f3460" }}>
                        {convertedPrice}
                      </span>
                      <span style={{ fontSize: "13px", color: "#dc3545", textDecoration: "line-through" }}>
                        {convertedOldPrice}
                      </span>
                    </div>
                    <div style={{ fontSize: "11px", color: "#888", marginBottom: "10px" }}>per person</div>
