...
export default function ReservationDetailDrawer({ reservation, onClose }) {
  ...
  const itemType = reservation.reservable_type?.split('\\').pop() || 'Item';
  const itemName =
    item.name_en ||
    item.name ||
    item.title ||
  ...
  const userName = reservation.user?.name || 'Unknown User';
      ...
      >
        <div className="h-full overflow-y-auto p-5">
          ...
          <section className="mb-6">
            <p className="text-xs uppercase text-slate-400 font-semibold mb-2">User Info</p>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{reservation.user?.email || 'No email'}</p>
          ...
          <section className="mb-6">
            <p className="text-xs uppercase text-slate-400 font-semibold mb-2">Item Info</p>
            <div className="space-y-2">
              ...
              {itemImage ? (
                ...
                <div className="w-full h-24 rounded-lg border border-slate-200 bg-slate-50 text-slate-400 flex items-center justify-center text-sm">No image</div>
              )}
              <p className="text-sm text-slate-500">{location || 'Location not specified'}</p>
            </div>
          </section>

          <section className="mb-6">
            <p className="text-xs uppercase text-slate-400 font-semibold mb-2">Reservation Details</p>
            <div className="space-y-1.5 text-sm">
              <p><span className="text-slate-500">Reservation ID:</span> #{reservation.id}</p>
              <p>
              ...
              <p><span className="text-slate-500">Date:</span> {reservation.date ? new Date(reservation.date).toLocaleDateString('en-GB') : 'Not specified'}</p>
              <p><span className="text-slate-500">Guests:</span> {reservation.people ?? 1}</p>
              <p><span className="text-slate-500">Created at:</span> {reservation.created_at ? new Date(reservation.created_at).toLocaleString('en-GB') : 'Not specified'}</p>
              <p><span className="text-slate-500">Cancelled at:</span> {reservation.cancelled_at ? new Date(reservation.cancelled_at).toLocaleString('en-GB') : '—'}</p>
            </div>
          </section>

          <section className="mb-6">
            <p className="text-xs uppercase text-slate-400 font-semibold mb-2">Booking Link</p>
            {bookingLink ? (
              ...
              <p className="text-sm text-slate-500">No booking link available</p>
            )}
          </section>

          <section>
            <p className="text-xs uppercase text-slate-400 font-semibold mb-2">Status Timeline</p>
            <div className="space-y-3">
              {[
                { key: 'saved', label: 'Saved', time: reservation.created_at },
                { key: 'opened', label: 'Link Opened', time: stepReached(reservation.status, 'opened') ? reservation.updated_at : null },
                {
                  key: 'response',
                  label: reservation.status === 'booking_claimed'
                    ? 'Booking Response: Booked'
                    : reservation.status === 'booking_declined'
                      ? 'Booking Response: Not Booked'
                      : reservation.status === 'left_without_booking'
                        ? 'Booking Response: No Response'
                        : 'Booking Response',
                  time: stepReached(reservation.status, 'response') ? reservation.updated_at : null,
                },
                { key: 'cancelled', label: 'Cancelled', time: reservation.cancelled_at },
              ].map((step) => {
